...
# save files cannot be shared and reused across players. 
# (You can change this by modifying the open_encrypted_with_pass function's 
# encryption key to something static)
...
const file_path : String = "user://localpersistentdata.sav"
# Note: If you change *use_encryption*, you have to delete your existing save file, since for example,
# if you saved with encryption on and then turned this off, Godot will try to parse it without 
# encryption, even though the file you have saved *is* encrypted!
...
func save():
	...
	file.store_string(JSON.print(current_state_dictionary))
	emit_signal("saved")
...
func _load():
	...
	if file.file_exists(file_path):
		current_state_dictionary = JSON.parse(file.get_as_text()).result
	emit_signal("loaded")
